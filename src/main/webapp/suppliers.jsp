...
<head>
    <meta charset="UTF-8">
    <title>CuraPharm — Supplier Records</title>
    <style>
        ...
        input[type="text"], input[type="email"], textarea { width: 100%; padding: 8px; border: 1px solid #bdc3c7; border-radius: 4px; box-sizing: border-box; }
        .btn-save { background-color: #3498db; color: white; padding: 10px 15px; border: none; border-radius: 4px; cursor: pointer; font-size: 15px; width: 100%; font-weight: bold; }
        .btn-update { background-color: #e67e22; color: white; padding: 10px 15px; border: none; border-radius: 4px; cursor: pointer; font-size: 15px; width: 100%; font-weight: bold; }
    ...
    <div class="card-form">
        ...
        <form action="<%= isEditMode ? "EditHandlerServlet" : "SupplierServlet" %>" method="POST">
            ...
            <div class="form-group">
                ...
                <input type="text" id="supplier_name" name="supplier_name" value="<%= editName %>" required>
            </div>
            <div class="form-group">
                <label for="contact_name">Contact Representative</label>
                <input type="text" id="contact_name" name="contact_name" value="<%= editContact %>">
            </div>
            <div class="form-group">
                <label for="phone">Phone Number</label>
                <input type="text" id="phone" name="phone" value="<%= editPhone %>">
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <textarea id="address" name="address" rows="2"><%= editAddress %></textarea>
            </div>
            <div class="form-group">
                <label for="bank_details">Payment / Bank Details</label>
                <input type="text" id="bank_details" name="bank_details" value="<%= editBank %>">
            </div>

            <% if (isEditMode) { %>
                <button type="submit" class="btn-update">Update Record</button>
                <p style="text-align:center;"><a href="suppliers.jsp" style="color:#7f8c8d; font-size:13px;">Cancel Edit Mode</a></p>
            <% } else { %>
            ...
            <tbody>
                <%
                    try (Connection conn = DBConnection.getConnection();
                         ...
                         ResultSet rs = stmt.executeQuery("SELECT * FROM Suppliers ORDER BY supplier_id DESC")) {
                        while(rs.next()) {
                %>
                <tr>
                    ...
                    <td>
                        ...
                        <a href="DeleteHandlerServlet?type=supplier&id=<%= rs.getInt("supplier_id") %>" class="delete-link" onclick="return confirm('Remove supplier profile?');">Delete</a>
                    </td>
                </tr>
                <%
                        }
